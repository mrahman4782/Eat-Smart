...
export async function removeMenuItem(session, itemId) {
    ...
    if (checkUserLogin.data.type == 'Chef') {
      try {
        ...
        response.status = 200;
        response.data = `Item with ID ${itemId} successfully deleted`;
